...
3. Realtime
- Add both tables to the Supabase realtime publication so the facilitator view
  receives live updates as delegates submit tokens and narratives.

4. Notes
- The `submission_cards` table stores one row per bridge+card combination, making aggregate queries straightforward.
- The `payload` jsonb column on `submissions` stores the complete response as a full-fidelity backup.
*/
...
ALTER PUBLICATION supabase_realtime ADD TABLE submission_cards;
